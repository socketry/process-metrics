...
- {ruby Process::Metrics::Sampler} computes CPU utilization over intervals from consecutive process snapshots.
...
A single process snapshot contains cumulative CPU time. Use {ruby Process::Metrics::Sampler} to calculate CPU utilization over an interval:

``` ruby
sampler = Process::Metrics::Sampler.new
